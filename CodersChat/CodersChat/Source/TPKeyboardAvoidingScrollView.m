...
-(BOOL)textFieldShouldReturn:(UITextField *)textField {
    if ( ![self focusNextTextField] ) {
     ...
     [[NSNotificationCenter defaultCenter] postNotificationName:@"EndTextFieldEditting" object:textField userInfo:nil];
    return YES;
}
...
- (void) textFieldDidEndEditing:(UITextField *)textField {
    //  [Pdelegate textEndEditing:textField];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"EndTextFieldEditting" object:textField userInfo:nil];
}
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range
replacementString:(NSString *)string {
    
    NSArray *myArray=[[NSMutableArray alloc]initWithObjects:[NSValue valueWithRange:range],string, nil];
    NSDictionary *theInfo =[NSDictionary dictionaryWithObjectsAndKeys:myArray,@"myArray", nil];
     [[NSNotificationCenter defaultCenter] postNotificationName:@"TextFieldshouldChangeCharactersInRange" object:textField userInfo:theInfo];
    return YES;
}
